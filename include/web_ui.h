...
// On ESP32 builds, hosts a WiFi access point and serves a tiny status page:
// one box per Raceband channel, green when free / red when busy, polling
// the bitmask set via setBusyMask(). On boards without WiFi (Nano/Uno/Pro
// Micro), all three functions are no-ops so main.cpp doesn't need
// per-board #ifdefs.
namespace WebUi {
